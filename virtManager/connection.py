...
class vmmConnection(vmmGObject):
    ...
    def _add_conn_events(self):
        ...
        def _add_storage_pool_refresh_event(eventid, typestr):
            if not self.using_storage_pool_events:
                return
            try:
                self._storage_pool_cb_ids.append(
                    self.get_backend().storagePoolEventRegisterAny(
                    None, eventid, self._storage_pool_refresh_event, None))
            except Exception, e:
                logging.debug("Error registering storage pool %s event: %s",
                    typestr, e)

        _add_storage_pool_refresh_event(
            getattr(libvirt, "VIR_STORAGE_POOL_EVENT_ID_REFRESH", 1),
            "refresh")
        try:
            if FORCE_DISABLE_EVENTS:
                raise RuntimeError("FORCE_DISABLE_EVENTS = True")
